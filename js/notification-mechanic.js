// ================================
// NOTIFICATION SYSTEM - MECHANIC VERSION
// CHỈ LOGIC, KHÔNG INJECT HTML
// ================================

document.addEventListener('DOMContentLoaded', function() {
    // Bell HTML đã có sẵn trong mechanic pages
    // Chỉ cần init logic thôi
    initMechanicNotificationLogic();
});

// ================================
// MECHANIC NOTIFICATION LOGIC
// ================================
function initMechanicNotificationLogic() {
    const API_URL = 'https://suaxeweb-production.up.railway.app';
    let currentTab = 'all';
    
    // DOM Elements
    const bellButton = document.getElementById('mechanicNotificationBell');
    const badge = document.getElementById('mechanicNotificationBadge');
    const dropdown = document.getElementById('mechanicNotificationDropdown');
    const notificationsList = document.getElementById('mechanicNotificationsList');
    const loadingState = document.getElementById('mechanicLoadingState');
    const emptyState = document.getElementById('mechanicEmptyState');
    const markAllReadBtn = document.getElementById('mechanicMarkAllRead');
    const tabButtons = document.querySelectorAll('.mechanic-tab-button');
    const unreadTabCount = document.getElementById('mechanicUnreadTabCount');
    
    // Check if elements exist
    if (!bellButton || !dropdown) {
        console.error('Mechanic notification bell elements not found');
        return;
    }
    
    console.log('✅ Mechanic notification system initialized');
    ...
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.mechanic-notification-bell')) {
    ...
    async function loadNotifications() {
        ...
        try {
            ...
            const url = `${API_URL}/api/notifications?limit=10&unread=${unreadOnly}`;
    ...
    function renderNotifications(notifications) {
        ...
        const html = notifications.map(notif => `
            <div class="notification-item ${notif.IsRead ? '' : 'unread'}" 
                 ...
                 onclick="handleMechanicNotificationClick(${notif.NotificationID}, '${notif.ActionUrl || ''}')">
    ...
    window.handleMechanicNotificationClick = async function(notificationId, actionUrl) {
    ...
    function showEmptyState(message) {
        ...
        if (emptyState) {
            ...
            if (emptyP) {
                emptyP.textContent = message || 'Không có thông báo';
            }
        }
    }
